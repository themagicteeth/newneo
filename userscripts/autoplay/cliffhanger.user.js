// ==UserScript==
// @name           Cliffhanger Autoplayer
// @namespace      Neoscripts
// @description    Autoplays Cliffhanger
// @match          *://www.neopets.com/games/cliffhanger/*
// @grant          GM_log
// ==/UserScript==
...
try {
	...
	let puzzle = `${blanksArea.innerHTML}`;
	...
	const regexp = new RegExp(`^${puzzle.replace(/_/g, '\\w')}$`);
	regexp.ignoreCase = true;

	for (let ans = 0; ans < answer.length; ans++) {
		if (answer[ans].length == puzzle.length && regexp.test(answer[ans])) {
