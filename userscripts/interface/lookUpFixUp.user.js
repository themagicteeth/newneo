...
// @description    Links the trophy images on someone's lookup to the game page. Counts the game trophies. Allows you to remove the user style.
// @include        *://www.neopets.com/userlookup.phtml?user=*
// @include        *://neopets.com/userlookup.phtml?user=*
// @version        1.0.0
// @grant          GM_addStyle
// ==/UserScript==
...
let trophyDiv = document.getElementById('usertrophies');
...
if (trophyDiv && !trophyDiv.textContent.match('Trophies: None')) {
    ...
    for (var x = 0, thisDiv; thisDiv = copyDiv.getElementsByTagName('div')[x]; x++) {
        if (thisDiv.textContent == '- Game Trophies -') {
            thisDiv.addEventListener('click', () => { document.getElementById('trophystats').setAttribute('style', ''); }, false);
            ...
            const tcount = acount + bcount + ccount + dcount;
            ...
            for (let y = 0, thisCell; thisCell = thisTable.getElementsByTagName('td')[y]; y++) {
